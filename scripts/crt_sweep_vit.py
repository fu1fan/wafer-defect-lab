#!/usr/bin/env python
"""cRT (classifier retraining) sweep for ViT backbones with logit adjustment stacking.

Freezes backbone, retrains head (fc + head norm) with focal loss.
Sweeps LR / gamma combos, then applies logit adjustment.

Usage:
    python scripts/crt_sweep_vit.py --checkpoint <path/to/best.pt> \
        --config <path/to/experiment.yaml> --output-dir <output_dir>
"""
...
from waferlab.data.dataloaders import build_classification_dataloaders, _build_class_balanced_sampler
...
from waferlab.models.resnet import FAILURE_TYPE_NAMES
...
def freeze_backbone_vit(model):
    """Freeze everything except fc.* and backbone.head.norm.* (the classifier head)."""
    ...
    print(f"  Frozen: {n_total - n_train:,} / {n_total:,} params. "
          f"Trainable: {n_train:,}")
...
def main():
    ...
    # Load config to get model arch and dataloaders
    config = load_yaml_config(args.config)
    data_config = load_data_config(
        PROJECT_ROOT / "configs" / "data" / "wm811k.yaml")
    ...
    # cRT experiment configs
    experiments = [
        {"name": "crt_lr1e-3_g1.0", "lr": 1e-3, "gamma": 1.0, "epochs": 4},
        {"name": "crt_lr1e-3_g1.5", "lr": 1e-3, "gamma": 1.5, "epochs": 4},
        {"name": "crt_lr5e-4_g1.0", "lr": 5e-4, "gamma": 1.0, "epochs": 4},
        {"name": "crt_lr2e-3_g1.0", "lr": 2e-3, "gamma": 1.0, "epochs": 6},
    ]
    ...
    for exp in experiments:
        ...
        model = MODEL_REGISTRY.build(arch, model_cfg)
        ...
        print(f"  Per-class recall (stacked): "
              f"Center={pcr['Center']:.3f} Loc={pcr['Loc']:.3f} "
              f"Scratch={pcr['Scratch']:.3f} E-Ring={pcr['Edge-Ring']:.3f}")
    ...
    print("SUMMARY")
    print(f"{'='*80}")
    print(f"\n{'Name':<25s} {'LR':>8s} {'γ':>5s} {'cRT':>8s} {'τ':>6s} "
          f"{'Stacked':>8s}")
    print("-" * 65)

    for name, r in all_results.items():
        print(f"{name:<25s} {r['lr']:>8.0e} {r['focal_gamma']:>5.1f} "
              f"{r['crt_score']:>8.4f} {r['best_logit_adj_tau']:>6.2f} "
              f"{r['stacked_score']:>8.4f}")
    ...
    print(f"\nBest: {best_name} → stacked_score={best_r['stacked_score']:.4f}")
